...
System opiera się na etapowo rozszerzanych danych statycznych (np. pobieranych
z OpenStreetMap), co zapewnia niezawodność testów i uniezależnia działanie symulacji
...
jak zamknięcie jednego, kluczowego węzła na Śląsku wpływa na kaskadowe opóźnienia
...
tor boczny, aby przepuścić skład priorytetowy).
...
obniża próg wejścia w złożony świat zarządzania ruchem kolejowym, pozwalając na
bezpieczne eksperymentowanie zarówno osobom nie związanym z koleją, jak i logistykom.
...
utrzymanie wysokiej responsywności aplikacji webowej w środowisku przeglądarkowym.
\end{itemize}
...
dynamicznych awarii oraz zmiennego obciążenia sieci. Zastosowano więc mechanizm
automatycznego wyznaczania dróg alternatywnych oparty na algorytmie heurystycznym A*
(A-star). Wykorzystuje on przeszukiwanie przestrzeni stanów grafu, co w literaturze
przedmiotu wskazywane jest jako najskuteczniejsze narzędzie do optymalizacji
przepływów w sieciach o strukturze rozproszonej~\cite{astar1968}.

W zaimplementowanym rozwiązaniu algorytm optymalizuje rzeczywisty czas przejazdu
pociągu wyrażony w minutach. Funkcja kosztu dotychczasowej ścieżki $g(n)$ uwzględnia
efektywną prędkość na danym odcinku toru -- wyznaczaną jako minimum z maksymalnej
prędkości szlaku oraz konstrukcyjnej prędkości pociągu
($v_{\text{efektywna}} = \min(v_{\text{toru}}, v_{\text{pociągu}})$) -- a także
ewentualne narzuty czasowe wynikające ze specyfiki taboru (np. postój pociągu
regionalnego na stacjach pośrednich) oraz opóźnienia liniowe ($delay\_min$). Aby
zagwarantować matematyczną poprawność i admisyjność (dopuszczalność) wyszukiwania,
algorytm zintegrowano ze wzorem Haversine'a~\cite{haversine}. Oblicza on odległość
sferyczną w linii prostej między bieżącą stacją a stacją docelową, która następnie
jest przeliczana na dolne oszacowanie czasu przejazdu przy maksymalnej prędkości
w sieci ($v_{\max} = 160\text{ km/h}$):
\begin{equation}
h(n) = \frac{\text{Haversine}(n, \text{cel})}{v_{\max}} \cdot 60
\end{equation}
Dzięki temu heurystyka nigdy nie przeszacowuje rzeczywistego czasu przejazdu
($h(n) \le h^*(n)$), co gwarantuje odnalezienie trasy optymalnej czasowo. Ponadto
algorytm operuje wyłącznie na odcinkach o statusie aktywnym (\texttt{status = 'active'}),
co umożliwia natychmiastowy rerouting w przypadku wystąpienia awarii lub zablokowania
toru.
...
\subsubsection{Struktura grafowego modelu danych}

Zaprojektowany model danych w bazie Memgraph opiera się na etykietach wierzchołków
oraz relacjach opisujących odcinki torów i tabor:
\begin{itemize}
\item \textbf{Wierzchołki stacji (\texttt{:Station}):} Reprezentują punkty węzłowe, stacje przelotowe
oraz krańcowe sieci kolejowej. Każdy węzeł posiada unikalny identyfikator (\texttt{id}, np. \texttt{KAT},
\texttt{GLI}), pełną nazwę (\texttt{name}), współrzędne geograficzne (\texttt{lat}, \texttt{lon})
wykorzystywane przez heurystykę Haversine'a, typ stacji (\texttt{type}: \emph{węzeł}, \emph{przelotowa},
\emph{końcowa}), liczbę peronów (\texttt{platforms}) i torów stacyjnych (\texttt{tracks}) oraz
statystykę dziennego natężenia ruchu (\texttt{daily\_trains}).
\item \textbf{Wierzchołki pociągów (\texttt{:Train}):} Modelują tabor kolejowy w symulacji,
zawierając m.in. unikalny kod pociągu (\texttt{id}), kategorię (\texttt{type}: \texttt{IC},
\texttt{regional}, \texttt{freight}), priorytet ruchowy, maksymalną prędkość konstrukcyjną
(\texttt{vmax}), masę, przyspieszenie oraz aktualny stan i trasę przejazdu.
\item \textbf{Relacje szlaków torowych (\texttt{:TRACK}):} Łączą węzły stacji dwukierunkowo,
reprezentując fizyczne odcinki linii kolejowych. Atrybuty relacji obejmują identyfikator
odcinka (\texttt{segment\_id}), numer linii (\texttt{line}), długość w kilometrach (\texttt{dist\_km}),
maksymalną dopuszczalną prędkość szlakową (\texttt{vmax}), bazowy czas przejazdu w minutach
(\texttt{travel\_min}), liczbę torów (\texttt{rail\_tracks}), stan operacyjny
(\texttt{status}: \texttt{'active'}) oraz narzut opóźnienia (\texttt{delay\_min}).
\end{itemize}
Dzięki takiemu odwzorowaniu grafowemu zapytania wyznaczające najszybszą trasę algorytmem A*
mogą w ułamku sekundy analizować zarówno parametry geometryczne sieci, jak i dynamiczne
ograniczenia eksploatacyjne poszczególnych odcinków.
